...
function Review(ID, author, date, comment, ratingService, ratingPrice, ratingValue, 
  ...
  this.setRating = (ratingService, ratingPrice, ratingValue, ratingQuality) => {
    this.rating.service = ratingService;
    this.rating.price = ratingPrice;
    this.rating.value = ratingValue;
    this.rating.quality = ratingQuality;
  };
}

/**
 * The function generates a unique ID code consisting of a set of 8 characters (letters and numbers)
 * @returns  Unique ID
 */
function generateId() {
...
function sortProducts(products, sortRule){
  ...
  for(let i = 1; i < products.length; i++){
    productFirst = products[i-1];
    productSecond = products[i];
    if(sortRule.toLowerCase() === "price" 
      && productSecond.getPrice() < productFirst.getPrice()){
        products.splice(i-1, 2, productSecond, productFirst);
        i = 0;
    }
    if(sortRule.toLowerCase() === "name" 
      && productSecond.getName().toLowerCase() < productFirst.getName().toLowerCase()){
        products.splice(i-1, 2, productSecond, productFirst);
        i = 0;
    }
    if(sortRule.toLowerCase() === "id" 
      && productSecond.getID() < productFirst.getID()){
        products.splice(i-1, 2, productSecond, productFirst);
        i = 0;
    }
  }
